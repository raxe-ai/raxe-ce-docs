...
| MCP gateway not starting | Node.js or npx issues | [MCP Gateway](#mcp-gateway) |
...
- Monitor for `detection_count: 0` in telemetry
...
# Expected: <10ms (L1 only), <50ms (L1+L2)
# Problem: >100ms
...
raxe = Raxe()
result = raxe.scan(prompt, layers=["l1"])  # L1 only: <10ms
...
**Symptoms:**
- `npx @anthropic-ai/raxe-mcp` fails to start
...
npm view @anthropic-ai/raxe-mcp

# Try running directly
npx @anthropic-ai/raxe-mcp --help
```
...
# Run with full path
$(npm bin -g)/npx @anthropic-ai/raxe-mcp

# Or install globally
npm install -g @anthropic-ai/raxe-mcp
raxe-mcp --help
```
...
### Upstream command issues

```bash
# Test MCP server standalone
echo '{"jsonrpc": "2.0", "method": "initialize", "id": 1}' | \
    npx @anthropic-ai/raxe-mcp
...
**Prevention:**
- Pin Node.js version in CI/CD
- Test MCP configuration before deploying
- Use absolute paths in config if npx has issues
