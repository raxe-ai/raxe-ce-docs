...
**Stop prompt injection, jailbreaks, and agent hijacking in real-time.** RAXE scans every prompt and response locally — no cloud, no data leaks, sub-5ms latency.
...
<Accordion title="How Detection Works">
  ...
  **L1 (Rules):** 515+ regex patterns across 11 threat families. Sub-millisecond latency. Catches known attack signatures.
...
<Tip>RAXE Community Edition includes 1,000 scans/day free — plenty for development and small deployments. Need more? [Contact us](/enterprise-contact) for Pro and Enterprise options.</Tip>
