...
```python title="background_mode.py"
from raxe import Raxe
...
scanner = create_agent_scanner(
    ...
    AgentScannerConfig(
        ...
        on_threat_callback=lambda r: print(
            f"Threat: severity={r.severity}, hash={r.prompt_hash}"
        ),
    ),
)

# Returns in <1ms — scan runs in background
scanner.scan_prompt("user input")
...
| Mode | Overhead | Threat Result | Use Case |
|------|----------|---------------|----------|
| `"sync"` (default) | ~5-200ms | Inline | When you need to block on threats |
| `"background"` | ~0ms | Via callback | Log-only monitoring, latency-sensitive apps |

<Note>
Background mode is incompatible with `on_threat="block"`. If both are set, RAXE auto-corrects to sync mode with a warning.
</Note>
