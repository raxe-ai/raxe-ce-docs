...
The `scan()` method returns a `ScanResult`:
...
# Properties
result.is_safe            # bool: True if no threats detected
result.is_threat          # bool: True if any threat detected
result.highest_severity   # Severity: CRITICAL/HIGH/MEDIUM/LOW/NONE
result.detection_count    # int: Number of detections
result.detections         # list[Detection]: All detections
result.scan_duration_ms   # float: Scan time in milliseconds
```
